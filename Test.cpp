...
array<int, 3> a = {-1,-2,-3};
vector<string> l = {"Very" , "Good" };
range r(1,3);


TEST_CASE("Basic Accumulate Tests")
{
    CHECK_NOTHROW(for(auto i: accumulate(vecInt)));
    CHECK_NOTHROW(for(auto i: accumulate(singleVecInt))); //9
    CHECK_THROWS(for(auto i: accumulate(emptyVecInt))); //10
    CHECK_NOTHROW(for(auto i: accumulate(vecString))); //11
    CHECK_NOTHROW(for(auto i: accumulate(vecDouble))); //12
    CHECK_NOTHROW(for(auto i: accumulate(a))); //13
    CHECK_NOTHROW(for(auto i: accumulate(l))); //14
    CHECK_NOTHROW(for(auto i: accumulate(r))); //15
...
TEST_CASE("Accumulate With Binary Operator")
{
    ...
    for(string i: accumulate(vecString,[](string x, string y){return x+y;}))
        ans_str+=i;
            CHECK(ans_str == "ThisThisIsThisIsWorking"); //38
...
TEST_CASE("Filter False") {
            ...
            CHECK_THROWS(filterfalse([](int i) { return i % 2 == 0; }, vecString)); //65
...
//    ans_str = "";
//    for (auto i: filterfalse([](string i) { return i + "Good" == "VeryGood"; }, l))
//        ans_str = ans_str + to_string(i) + " ";
//            CHECK(ans_str == "Very "); //71
...
TEST_CASE("Compress"){
    ...
    vector<bool> ftf = {false, true, false};
   ...
   //CHECK_NOTHROW(compress(vecInt, ttt)); //81
    ...
    CHECK_THROWS(compress(string("abc"), tft)); //87
    CHECK_THROWS(compress(string("abcd"), tff)); //88
    CHECK_THROWS(compress(string("abc"), fft)); //87
//    CHECK_THROWS(compress()); //99
    CHECK_THROWS(compress(string("gh"), tf)); //100

    ans_str ="";
    for(auto i: compress(  string("abc"), tff  ))
        ans_str = ans_str + i + " ";
            CHECK(ans_str == "a "); //88

    ans_str ="";
    for(auto i: compress(  string("abc"), fff  ))
        ans_str = ans_str + i + " ";
            CHECK(ans_str == ""); //89

    ans_str ="";
    for(auto i: compress(  string("abc"), ttt  ))
        ans_str = ans_str + i + " ";
            CHECK(ans_str == "a b c "); //90

    ans_str ="";
    for(auto i: compress(  string("abc"), tft  ))
        ans_str = ans_str + i + " ";
            CHECK(ans_str == "a c "); //91

//    ans_str ="";
//    for(int i: compress(  vecInt, ttt  ))
//        ans_str = ans_str + to_string(i) + " ";
//            CHECK(ans_str == "1 2 3 "); //92
